...
string checkSix(string var,char LabelOrNum){
    ...
    if(sizee<6)
        if(LabelOrNum =='L')
            for(int i=sizee;i<6;i++)
                var = var + " ";
        else if(LabelOrNum =='N')
            for(int i=0;i < 6- sizee ;i++){
                var = "0" + var;
                //cerr << ">>>>>>>>" << i << "    " << var << endl ;
            }
    return var;
...
void ObjectCode(vector<compLine> &pass1,
    ...
    //else if(data_types.find(instruction) != data_types.end()){
...
string ret_HEX_6(int num){
	string hexa;
	stringstream ss;
	ss << hex << uppercase << num;
	hexa += ss.str();
	hexa = checkSix(hexa,'N');
	return hexa;
}

string check2Hex(int num){
    string hexa;
	stringstream ss;
	ss << hex << uppercase << num;
	hexa += ss.str();
...
vector<string> T_Record(vector<compLine> &pass1_1){
	vector<string> ans;
	int recInd = 0;
	int pass1_sz = pass1_1.size();
	for(recInd = 0; recInd < pass1_sz; recInd++){
		if(pass1_1[recInd].object_code == "") continue;
		string T = "T." + ret_HEX_6(pass1_1[recInd].address.first)+".";
		string objs;
		do{
			objs += pass1_1[recInd++].object_code;
		}while(recInd < pass1_sz && objs.length()/2 < 30 && pass1_1[recInd].object_code != "");
		if(objs.length()/2 >= 30) recInd--;
		T += (check2Hex(objs.length()/2) + ".");
		T += objs;
		ans.push_back(T);
	}
	return ans;
...
string get_T_records(vector<compLine> &pass1_1){
    vector<string> ans = T_Record(pass1_1);
    string ret;
	int ans_sz = ans.size();
...
string getDRecord(vector<pair<string,int>> &ExDef){
    string DRecord = "";
    if(ExDef.size()!=0){
        DRecord = DRecord + "D.";
        for(int i=0;i<ExDef.size();i++)
            DRecord = DRecord + checkSix(ExDef[i].first,'L') + checkSix(intToHex(ExDef[i].second,0),'N') + ".";
        return DRecord + "\n" ;
    }
    return DRecord ;
}

string getRRecord(vector<string> &ExRef){
    string RRecord="";
    if(ExRef.size()!=0){
           RRecord=RRecord+"R";
        for(int i=0;i<ExRef.size();i++)
            RRecord = RRecord + checkSix(ExRef[i],'L')+".";
        return RRecord +"\n";
    }
    return RRecord ;
}

string HTE(vector<compLine> &pass1,
           vector<Block> &blocks , vector<pair<string,int>> &ExDef , vector<string> &ExRef ){
    ...
    string rRecord =  getRRecord(ExRef);
    string dRecord =  getDRecord(ExDef);
    string tRecord =  get_T_records(pass1);
    //string mRecord = generateMRecord(pass1,dictionary); //ttzabt
    return hRecord + dRecord + rRecord + tRecord + eRecord ;
}

bool found_extref_by_name(string name, vector<string> &EXTREF){
	int ext_sz = EXTREF.size();
	for(int i = 0; i < ext_sz; i++){
		if(EXTREF[i] == name) return true;
	}
	return false;
}

vector<string> get_modif_extref(string operands, vector<string> &EXTREF ){
	int op_sz = operands.length();
	vector<string> ans;
	string val, oper = "+";
	for(int i = 0; i < op_sz; i++){
		if(operands[i] == '+' || operands[i] == '-' || operands[i] == ','){
			if(found_extref_by_name(val, EXTREF)){
				string temp = oper+val;
				ans.push_back(temp);
			}
			oper = operands[i];
			val = "";
...
int main(){
    ...
    print_pass1( pass1_1 );
    cout << HTE(pass1_1,blocks1,EXTDEF1,EXTREF1) << endl;

    vector<string> test = get_modif_extref("BUFEND-BUFFER", EXTREF1);
